...
const quickLinksData = [
  ...
  },
  {
    ...
    links: [
      { name: 'EdTech Summit 2025', nameKh: 'កិច្ចប្រជុំ EdTech ២០២៥', url: '/edtech-s2025', description: 'Annual technology in education conference' },
      { name: 'EdTech Summit 2026', nameKh: 'កិច្ចប្រជុំ EdTech ២០២៦', url: '/our-work/edtech-summit/s2026', description: 'Next generation educational technology showcase' },
  ...
  },
  {
    ...
    links: [
      ...
      { name: 'EdTech Members', nameKh: 'សមាជិក EdTech', url: '/register', description: 'View all EdTech members' },
...
function QuickLinksPopup({ isOpen, onClose }) {
  ...
  const modalContent = (
    <>
      {/* Backdrop with Blur */}
      <div 
        className="fixed inset-0 bg-black/70 backdrop-blur-md z-[999] transition-all duration-300"
        onClick={onClose}
      ...
      <div 
        className="fixed top-0 left-0 right-0 bottom-0 z-[1000] flex items-center justify-center p-2 sm:p-4 md:p-8"
        style={{ margin: 0, padding: '1rem' }}
      >
        <div 
          className="bg-white shadow-2xl w-full max-w-6xl h-auto max-h-[90vh] flex flex-col rounded-2xl overflow-hidden"
          style={{ position: 'relative' }}
          onClick={(e) => e.stopPropagation()}
        >
          {/* Header with Logo */}
          <div className="bg-blue-600 p-4 md:p-6 text-white flex-shrink-0">
            <div className="flex items-center justify-between gap-4">
              <div className="flex items-center space-x-3 md:space-x-6 flex-1 min-w-0">
                <img 
                  src={logo} 
                  alt="EdTech Cambodia" 
                  className="h-12 md:h-16 w-auto bg-transparent p-2 flex-shrink-0"
                />
                <div className="min-w-0">
                  <p className="text-blue-100 text-base md:text-lg lg:text-xl font-semibold truncate">Quick Links - EdTech Cambodia</p>
                  <p className="text-blue-200 text-xs md:text-sm mt-1">Navigate easily to all sections</p>
                </div>
              </div>
              <button
                onClick={onClose}
                className="p-2 md:p-3 hover:bg-yellow-400 rounded-full transition-all duration-200 hover:scale-110 hover:rotate-90 flex-shrink-0"
                aria-label="Close"
              >
          ...
          {/* Footer */}
          <div className="bg-gradient-to-r from-gray-50 to-gray-100 px-4 md:px-6 lg:px-8 py-3 md:py-4 border-t border-gray-200 flex-shrink-0">
            <div className="flex items-center justify-center text-sm">
              <p className="text-center text-gray-600 flex flex-wrap items-center justify-center gap-1">
                <span>Need help? Contact us at</span>
                <a 
                  href="mailto:info@edtechcambodia.org"
  ...
  return (
    <>
      <button 
        ...
        className="flex items-center space-x-1 text-white hover:text-yellow-400 transition-colors duration-200 font-semibold"
      >
        <span className="khmer-text">Quick Links</span>
      </button>
